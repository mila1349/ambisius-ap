...
const Dropzoneimg = (props : DropProps) => {
    return (
     <Dropzone onDrop={acceptedFiles => props.setFile(acceptedFiles)} accept={{'image/jpeg': ['.jpeg', '.png']}} maxFiles={props.maxFile}>
     ...
     <section>
          ...
          <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-40 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
               <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG or JPEG (MAX. 1 File)</p>
                    {
                         props.file?.map(item=>(
                              <p className="text-md text-red-500 dark:text-gray-400">{item.name}</p>
                         ))
                    }
               </div>
          </label>
